...
int WINAPI wWinMain(HINSTANCE _hInstance, HINSTANCE _hPrevInstance, LPWSTR _lpCmdLine, int _nCmdShow)
{
	...
	// Init the ViewerHandler with the window and config file
	ViewerHandler ViewerHandler(hwnd, configFile);
	// Forget the config file, the ViewerHandler is responsible for it now.
	configFile = NULL;

	return ViewerHandler.run();
}
