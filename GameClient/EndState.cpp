...
EndState::EndState(NetworkEndGameMessage endMessage,MissionEndMessage mem) : State(State::END)
{
	...
	else
	{
		...
		for(int i = 0; i < this->m_nrOfPlayers && i < 4; i++)
		{
			...
			this->m_playerInfo[i] = g_graphicsEngine->createMyText("text3.png", "text/", "offsets.txt", ss.str(), INT2(120+jumpColumn*400,325 + jumpRow * 170), 75);
			ss.clear();
			ss.str("");

			ss<<"Demons killed: "<<endMessage.getPlayers()[i].getDeamonsKilled();
			this->m_playerMonstersKilled[i] = g_graphicsEngine->createMyText("text3.png", "text/", "offsets.txt", ss.str(), INT2(120+jumpColumn*400,375 + jumpRow * 150), 50);
			ss.clear();
			ss.str("");
			...
			this->m_playerDamageDone[i] = g_graphicsEngine->createMyText("text3.png", "text/", "offsets.txt", ss.str(), INT2(120+jumpColumn*400,405 + jumpRow * 150), 50);
			ss.clear();
			ss.str("");
			...
			this->m_playerDamageTaken[i] = g_graphicsEngine->createMyText("text3.png", "text/", "offsets.txt", ss.str(), INT2(120+jumpColumn*400,435 + jumpRow * 150), 50);
			ss.clear();
			ss.str("");

			ss<<"Resources collected: "<<endMessage.getPlayers()[i].getGoldCollected();
			this->m_playerResourcesCollected[i] = g_graphicsEngine->createMyText("text3.png", "text/", "offsets.txt", ss.str(), INT2(120+jumpColumn*400,465 + jumpRow * 150), 50);
			ss.clear();
			ss.str("");

			ss << "Healed for: " << endMessage.getPlayers()[i].getHealedAmount();
			this->m_healedFor[i] =  g_graphicsEngine->createMyText("text3.png", "text/", "offsets.txt", ss.str(), INT2(120+jumpColumn*400,495 + jumpRow * 150), 50);
			ss.clear();
			ss.str("");
