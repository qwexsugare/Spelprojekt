		...
		for(int i = 1; i < 10; i++)
		{
			...
			if(outOfBounds(temp))
			{
				...
				if( t < (temp2 - FLOAT3(32,0,32)).length())
					avoidDir = cross/(0.1f*i*avoidBuffer);
				else
					avoidDir = FLOAT3(0,0,0) - cross/(0.1f*i*avoidBuffer);
			...
			if(avoidBuffer <t)
			{
				...
				avoidDir = avoidDir/(0.08f);//*(max(1,i/2)));//*avoidBuffer);//*(stat->getPosition() - m_position).length());
				return avoidDir;
			}
		}
