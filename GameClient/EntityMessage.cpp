...
EntityMessage::EntityMessage(unsigned int id, unsigned int modelId, FLOAT3 pos, FLOAT3 rot)
{
	this->id = id;
...
{
	return packet<<"ENT"<<e.id<<e.modelId<<e.pos.x<<e.pos.y<<e.pos.z<<e.rot.x<<e.rot.y<<e.rot.z;
}
sf::Packet& operator>>(sf::Packet& packet, EntityMessage& e)
{
	return packet >>e.id>>e.modelId>> e.pos.x>>e.pos.y>>e.pos.z>>e.rot.x>>e.rot.y>>e.rot.z;
}
