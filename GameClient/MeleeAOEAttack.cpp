...
bool MeleeAOEAttack::activate(unsigned int _targetId, unsigned int _senderId)
{
	...
	if(caster != NULL && target != NULL)
	{
		...
		for(int i = 0; i < entities.size(); i++)
		{
			...
			{
				EntityHandler::addEntity(new DelayedDamage(_senderId, entities[i]->getId(), 0.5f, caster->getPhysicalDamage(), caster->getMentalDamage(), Skill::AOE_MELEE_ATTACK, 2.0f / caster->getAttackSpeed()));
			}
		}
	}
