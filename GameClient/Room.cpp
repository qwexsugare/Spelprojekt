...
Room::Room(int _index, int _charId, string _roomDiffuse, string _roomGlow, FLOAT3 _pos, float _roomWidth)
{
	...
	this->m_fire = g_graphicsEngine->createParticleEngine("fire", D3DXVECTOR4(_pos.x + (_roomWidth*_index), _pos.y + 0.5f, _pos.z - 1.0f, 1), D3DXQUATERNION(0, 0, 0, 1), D3DXVECTOR2(1, 1));
}
