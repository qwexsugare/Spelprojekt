...
void TowerPlacer::place(Skill::SKILLS _towerType, const FLOAT3& _pos)
{
	...
	case Skill::FROST_TURRET:
		TowerPlacer::m_frostTurretObb->Center = XMFLOAT3(_pos.x, _pos.y, _pos.z);
		for(int i = 0; i < entities.size(); i++)
		{
	...
	case Skill::TESLA_CHAIN_TURRET:
		TowerPlacer::m_teslaChainTurretObb->Center = XMFLOAT3(_pos.x, _pos.y, _pos.z);
		for(int i = 0; i < entities.size(); i++)
		{
	...
	case Skill::POISON_TURRET:
		TowerPlacer::m_poisonTurretObb->Center = XMFLOAT3(_pos.x, _pos.y, _pos.z);
		for(int i = 0; i < entities.size(); i++)
		{
