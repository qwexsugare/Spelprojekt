...
FullScreenPlane::FullScreenPlane(ID3D10Device *device, ID3D10Texture2D *_texture)
{
	...
	vertexData[2].pos = D3DXVECTOR3(position.x, position.y + size.y, 0.0f);
	vertexData[2].texCoord = D3DXVECTOR2(0.0f, 1.0f);

	vertexData[3].pos = D3DXVECTOR3(position.x + size.x, position.y + size.y, 0.0f);
	...
	vertexData[5].pos = D3DXVECTOR3(position.x + size.x, position.y, 0.0f);
