...
QuadTreeNode::QuadTreeNode(int _levels, D3DXVECTOR2 _min, D3DXVECTOR2 _max, Camera* _camera)
{
	this->m_min = _min;
	this->m_max = _max;
	...
	if(_levels > 1)
	{
		this->m_children[0] = new QuadTreeNode(_levels-1, _min, (_max+_min)/2.0f, m_camera);
		this->m_children[1] = new QuadTreeNode(_levels-1, D3DXVECTOR2((_max.x+_min.x)/2.0f, _min.y), D3DXVECTOR2(_max.x, (_max.y+_min.y)/2.0f), m_camera);
		this->m_children[2] = new QuadTreeNode(_levels-1, D3DXVECTOR2(_min.x, (_max.y+_min.y)/2.0f), D3DXVECTOR2((_max.x+_min.x)/2.0f, _max.y), m_camera);
		this->m_children[3] = new QuadTreeNode(_levels-1, (_max+_min)/2.0f, _max, m_camera);
	}
	else
	{
		for(int i = 0; i < 4; i++)
...
void QuadTreeNode::getModels(stack<Model*>& _models, D3DXVECTOR2 _focalPoint)const
{
	...
	if(abs(_focalPoint.x - m_obb->Center.x) < m_obb->Extents.x+m_camera->getXOffset() && abs(_focalPoint.y - m_obb->Center.z) < m_obb->Extents.z+m_camera->getZOffset())
	{
		if(this->m_children[0])
		{
