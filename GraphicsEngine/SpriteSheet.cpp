...
SpriteSheet::SpriteSheet(DeviceHandler *deviceHandler, ID3D10ShaderResourceView* _texture, INT2 _position, INT2 _size, INT2 _nrOfFrames)
{
	...
	for(int y = 0; y < this->m_nrOfFrames.y; y++)
	{
		for(int x = 0; x < this->m_nrOfFrames.x; x++)
		{
			...
			vertexData[i].texCoord = D3DXVECTOR2(x / this->m_nrOfFrames.x, (y + 1.0f) / this->m_nrOfFrames.y);
			i++;

			vertexData[i].pos = D3DXVECTOR3(pos.x + this->m_size.x, pos.y, 0.0f);
			vertexData[i].texCoord = D3DXVECTOR2((x + 1.0f) / this->m_nrOfFrames.x, y / this->m_nrOfFrames.y);
			i++;

			vertexData[i].pos = D3DXVECTOR3(pos.x, pos.y, 0.0f);
			vertexData[i].texCoord = D3DXVECTOR2(x / this->m_nrOfFrames.x, y / this->m_nrOfFrames.y);
			i++;
			...
			vertexData[i].texCoord = D3DXVECTOR2((x + 1.0f) / this->m_nrOfFrames.x, (y + 1.0f) / this->m_nrOfFrames.y);
			i++;
